  ...
  return <div className={`app-shell theme-${themeMode}`}>
    ...
    <main className="workspace">
      ...
      <section className="content">
        ...
        {activeTab === 'storage' && <Storage disk={disk} scanDisks={scanDisks} scanningDisks={scanningDisks} API_URL={API_URL} authenticated={authenticated} />}
...
function Storage({ disk, scanDisks, scanningDisks, API_URL, authenticated }: any) {
  const usage = percent(disk.percent), fileInput = useRef<HTMLInputElement>(null);
  const [files, setFiles] = useState<any[]>([]), [loadingFiles, setLoadingFiles] = useState(true), [uploading, setUploading] = useState(false), [fileError, setFileError] = useState('');
  const loadFiles = async () => { setLoadingFiles(true); try { const response = await fetch(`${API_URL}/storage/files`, { headers: authenticated() }); if (!response.ok) throw new Error('Unable to load files'); setFiles(await response.json()); setFileError(''); } catch (error) { setFileError(error instanceof Error ? error.message : 'Unable to load files'); } finally { setLoadingFiles(false); } };
  useEffect(() => { loadFiles(); }, [API_URL]);
  const uploadFile = async (event: React.ChangeEvent<HTMLInputElement>) => { const file = event.target.files?.[0]; if (!file) return; setUploading(true); setFileError(''); try { const body = new FormData(); body.append('file', file); const response = await fetch(`${API_URL}/storage/files`, { method: 'POST', headers: authenticated(), body }); if (!response.ok) throw new Error((await response.json().catch(() => ({}))).detail || 'Upload failed'); await loadFiles(); } catch (error) { setFileError(error instanceof Error ? error.message : 'Upload failed'); } finally { setUploading(false); event.target.value = ''; } };
  const downloadFile = async (name: string) => { try { const response = await fetch(`${API_URL}/storage/files/${encodeURIComponent(name)}`, { headers: authenticated() }); if (!response.ok) throw new Error('Download failed'); const url = URL.createObjectURL(await response.blob()), link = document.createElement('a'); link.href = url; link.download = name; link.click(); URL.revokeObjectURL(url); } catch (error) { setFileError(error instanceof Error ? error.message : 'Download failed'); } };
  const deleteFile = async (name: string) => { if (!confirm(`Delete ${name}? This cannot be undone.`)) return; try { const response = await fetch(`${API_URL}/storage/files/${encodeURIComponent(name)}`, { method: 'DELETE', headers: authenticated() }); if (!response.ok) throw new Error('Delete failed'); setFiles(current => current.filter(file => file.name !== name)); } catch (error) { setFileError(error instanceof Error ? error.message : 'Delete failed'); } };
  return <><PageHeader eyebrow="Storage management" title="Storage at a glance" description="Upload files from this device to /home/arsani/homelab/data/storage/." action={<div className="storage-actions"><button onClick={scanDisks} disabled={scanningDisks} className="outline-button"><HardDrive size={16} /> {scanningDisks ? 'Scanning...' : 'Scan disks'}</button><button onClick={() => fileInput.current?.click()} disabled={uploading} className="primary-small"><Upload size={16} /> {uploading ? 'Uploading...' : 'Upload file'}</button><input ref={fileInput} className="visually-hidden" type="file" onChange={uploadFile} /></div>} /><div className="storage-layout"><div className="panel storage-hero"><span className="storage-icon"><HardDrive size={25} /></span><h2>Primary volume</h2><p>{disk.mountpoint || '/'} · ext4 filesystem</p><div className="storage-number"><b>{byte(disk.used)}</b><span>used of {byte(disk.total)}</span></div><div className="bar"><i style={{ width: `${usage}%` }} /></div><div className="storage-foot"><span>{usage}% in use</span><span>{byte((disk.total || 0) - (disk.used || 0))} available</span></div></div><div className="panel disk-health"><PanelTitle title="Disk health" subtitle="SMART status & device details" /><div className="health-row"><span><i className="good-dot" /> SMART status</span><b>Passed</b></div><div className="health-row"><span>Temperature</span><b>34°C</b></div><div className="health-row"><span>Estimated life</span><b>98%</b></div><div className="health-row"><span>Last check</span><b>Today, 10:42</b></div></div></div><div className="panel storage-files"><PanelTitle title="Uploaded files" subtitle={`${files.length} file${files.length === 1 ? '' : 's'} in the storage area`} action={<button onClick={loadFiles} className="text-button"><RefreshCw size={14} /> Refresh</button>} />{fileError && <p className="storage-error">{fileError}</p>}{loadingFiles ? <div className="empty"><RefreshCw className="spinning" size={20} /><span>Loading files…</span></div> : files.length === 0 ? <div className="empty"><Upload size={22} /><b>No uploaded files yet</b><span>Choose “Upload file” to add data from your computer.</span></div> : <div className="file-list">{files.map(file => <div className="file-row" key={file.name}><span className="file-icon"><FileText size={17} /></span><span><b>{file.name}</b><small>{byte(file.size)} · {new Date(file.modified * 1000).toLocaleString()}</small></span><div className="row-actions"><button onClick={() => downloadFile(file.name)} title={`Download ${file.name}`}><ArrowDownRight size={15} /></button><button onClick={() => deleteFile(file.name)} title={`Delete ${file.name}`}><Trash2 size={15} /></button></div></div>)}</div>}</div></>;
}
