...
import {
  ...
  ChevronRight, CircleHelp, Clock3, Command, Cpu, FileText, Folder, FolderPlus, HardDrive,
  KeyRound, LayoutDashboard, LogOut, Menu, MoreHorizontal, Network,
  Play, Plus, RefreshCw, Search, Server, Settings, ShieldCheck,
  Square, Terminal, Trash2, Upload, User, X, Zap
} from 'lucide-react';
import './App.css';
...
export default function App() {
  ...
  const [, setScanningDisks] = useState(false);
...
function Storage({ API_URL, authenticated, disks, refreshDisks }: any) {
  const fileInput = useRef<HTMLInputElement>(null);
  const [files, setFiles] = useState<any[]>([]), [directory, setDirectory] = useState(''), [loadingFiles, setLoadingFiles] = useState(true), [uploading, setUploading] = useState(false), [fileError, setFileError] = useState(''), [editingPath, setEditingPath] = useState(''), [editorContent, setEditorContent] = useState(''), [saving, setSaving] = useState(false);
  const loadFiles = async (folder = directory) => { setLoadingFiles(true); try { const response = await fetch(`${API_URL}/storage/files?directory=${encodeURIComponent(folder)}`, { headers: authenticated() }); if (!response.ok) throw new Error('Unable to load files'); setFiles(await response.json()); setFileError(''); } catch (error) { setFileError(error instanceof Error ? error.message : 'Unable to load files'); } finally { setLoadingFiles(false); } };
  useEffect(() => { loadFiles(); }, [API_URL, directory]);
  const uploadFile = async (event: React.ChangeEvent<HTMLInputElement>) => { const file = event.target.files?.[0]; if (!file) return; setUploading(true); setFileError(''); try { const body = new FormData(); body.append('file', file); const response = await fetch(`${API_URL}/storage/files?directory=${encodeURIComponent(directory)}`, { method: 'POST', headers: authenticated(), body }); if (!response.ok) throw new Error((await response.json().catch(() => ({}))).detail || 'Upload failed'); await loadFiles(); } catch (error) { setFileError(error instanceof Error ? error.message : 'Upload failed'); } finally { setUploading(false); event.target.value = ''; } };
  const createFolder = async () => { const name = prompt('Folder name'); if (!name) return; try { const response = await fetch(`${API_URL}/storage/folders?directory=${encodeURIComponent(directory)}&name=${encodeURIComponent(name)}`, { method: 'POST', headers: authenticated() }); if (!response.ok) throw new Error((await response.json().catch(() => ({}))).detail || 'Could not create folder'); await loadFiles(); } catch (error) { setFileError(error instanceof Error ? error.message : 'Could not create folder'); } };
  const downloadFile = async (path: string, name: string) => { try { const response = await fetch(`${API_URL}/storage/files/${path.split('/').map(encodeURIComponent).join('/')}`, { headers: authenticated() }); if (!response.ok) throw new Error('Download failed'); const url = URL.createObjectURL(await response.blob()), link = document.createElement('a'); link.href = url; link.download = name; link.click(); URL.revokeObjectURL(url); } catch (error) { setFileError(error instanceof Error ? error.message : 'Download failed'); } };
  const deleteFile = async (file: any) => { if (!confirm(`Delete ${file.name}?${file.type === 'folder' ? ' Folders must be empty.' : ''}`)) return; try { const response = await fetch(`${API_URL}/storage/files/${file.path.split('/').map(encodeURIComponent).join('/')}`, { method: 'DELETE', headers: authenticated() }); if (!response.ok) throw new Error((await response.json().catch(() => ({}))).detail || 'Delete failed'); await loadFiles(); } catch (error) { setFileError(error instanceof Error ? error.message : 'Delete failed'); } };
  const openEditor = async (path: string) => { try { const response = await fetch(`${API_URL}/storage/text/${path.split('/').map(encodeURIComponent).join('/')}`, { headers: authenticated() }); if (!response.ok) throw new Error((await response.json().catch(() => ({}))).detail || 'Unable to open file'); setEditorContent((await response.json()).content); setEditingPath(path); } catch (error) { setFileError(error instanceof Error ? error.message : 'Unable to open file'); } };
  const saveEditor = async () => { setSaving(true); try { const response = await fetch(`${API_URL}/storage/text/${editingPath.split('/').map(encodeURIComponent).join('/')}`, { method: 'PUT', headers: authenticated({ 'Content-Type': 'application/json' }), body: JSON.stringify({ content: editorContent }) }); if (!response.ok) throw new Error((await response.json().catch(() => ({}))).detail || 'Save failed'); setEditingPath(''); await loadFiles(); } catch (error) { setFileError(error instanceof Error ? error.message : 'Save failed'); } finally { setSaving(false); } };
  const crumbs = directory ? directory.split('/') : [];
  return <><PageHeader eyebrow="Storage management" title="Home file browser" description="Browse and manage your full /home/arsani directory." action={<div className="storage-actions"><button onClick={createFolder} className="outline-button"><FolderPlus size={16} /> New folder</button><button onClick={() => fileInput.current?.click()} disabled={uploading} className="primary-small"><Upload size={16} /> {uploading ? 'Uploading...' : 'Upload file'}</button><input ref={fileInput} className="visually-hidden" type="file" onChange={uploadFile} /></div>} /><div className="panel disk-manager"><PanelTitle title="Disk manager" subtitle="Connected and mounted filesystems" action={<button onClick={refreshDisks} className="text-button"><RefreshCw size={14} /> Refresh</button>} />{disks.length === 0 ? <div className="empty"><HardDrive size={22} /><span>No accessible disks found.</span></div> : <div className="disk-list">{disks.map((item: any) => <div className="disk-row" key={`${item.device}-${item.mountpoint}`}><span className="file-icon"><HardDrive size={17} /></span><span><b>{item.device}</b><small>{item.mountpoint} · {item.fstype || 'Unknown filesystem'}</small></span><span className="disk-usage"><b>{byte(item.used)} / {byte(item.total)}</b><small>{Math.round(item.percent)}% used · {byte(item.free)} free</small></span><span className={`badge ${item.smart?.status === 'Healthy' ? 'success' : 'neutral'}`}>{item.smart?.status || 'Unknown'}</span></div>)}</div>}<p className="disk-note">Disk actions are intentionally read-only. Mounting, partitioning, and formatting are destructive operations and need explicit confirmation.</p></div><div className="panel storage-files"><div className="file-toolbar"><div className="breadcrumbs"><button onClick={() => setDirectory('')}>Home</button>{crumbs.map((crumb, index) => <span key={`${crumb}-${index}`}>/ <button onClick={() => setDirectory(crumbs.slice(0, index + 1).join('/'))}>{crumb}</button></span>)}</div><button onClick={() => loadFiles()} className="text-button"><RefreshCw size={14} /> Refresh</button></div>{fileError && <p className="storage-error">{fileError}</p>}{loadingFiles ? <div className="empty"><RefreshCw className="spinning" size={20} /><span>Loading files…</span></div> : files.length === 0 ? <div className="empty"><Upload size={22} /><b>This folder is empty</b><span>Upload a file or create a folder to get started.</span></div> : <div className="file-list">{files.map(file => <div className="file-row" key={file.path}><button className="file-entry" onClick={() => file.type === 'folder' ? setDirectory(file.path) : openEditor(file.path)}><span className="file-icon">{file.type === 'folder' ? <Folder size={17} /> : <FileText size={17} />}</span><span><b>{file.name}</b><small>{file.type === 'folder' ? 'Folder' : byte(file.size)} · {new Date(file.modified * 1000).toLocaleString()}</small></span></button><div className="row-actions">{file.type === 'file' && <><button onClick={() => openEditor(file.path)} title={`Edit ${file.name}`}><FileText size={15} /></button><button onClick={() => downloadFile(file.path, file.name)} title={`Download ${file.name}`}><ArrowDownRight size={15} /></button></>}<button onClick={() => deleteFile(file)} title={`Delete ${file.name}`}><Trash2 size={15} /></button></div></div>)}</div>}</div>{editingPath && <div className="modal-backdrop"><div className="modal-content editor-modal"><div className="modal-header"><h2><FileText size={18} /> {editingPath}</h2><button className="modal-close" onClick={() => setEditingPath('')}><X size={18} /></button></div><textarea className="file-editor" value={editorContent} onChange={event => setEditorContent(event.target.value)} spellCheck={false} /><div className="modal-footer"><button className="outline-button" onClick={() => setEditingPath('')}>Cancel</button><button className="primary-small" onClick={saveEditor} disabled={saving}>{saving ? 'Saving...' : 'Save file'}</button></div></div></div>}</>;
}
